 ...
 * @brief A header file for the CircularBuffer class template.
 * 
 * This file contains the definition and implementation of the CircularBuffer class template,
 * which provides a circular buffer (also known as a ring buffer) data structure.
 * 
 * @note This file is part of the CoffeeEngine project.
 * @note The original code is from https://github.com/vinitjames/circularbuffer/tree/master and is licensed under the MIT License.
 * 
 * @tparam T The type of elements stored in the circular buffer.
 * 
 * @class CircularBuffer
 * @brief A circular buffer (ring buffer) implementation.
 * 
 * The CircularBuffer class template provides a circular buffer data structure, which allows
 * for efficient insertion and removal of elements in a fixed-size buffer. The buffer operates
 * in a FIFO (First-In-First-Out) manner.
 * 
 * @note The class is thread-safe, using a mutex to protect concurrent access.
 * 
 * @section Example
 * @code
 ...
 * buffer.push_back(2);
 * buffer.push_back(3);
 * int front = buffer.front(); // front == 1
 * buffer.pop_front();
 * @endcode
 * 
 * @section License
